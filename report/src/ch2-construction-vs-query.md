...
Any kind of heuristics or caching strategy to speed up construction will result in a sub optimal partitioning of the space which the querying side will pay for. Therefore it is not worth it. Instead focus on speeding up the construction of an optimal partitioning. This can be done via parallelism. Sorting the bots that sit on dividing lines may seem slow, but we can get this for 'free' essentially because it can be done after we have already split up the children nodes. So we can finish sorting a node while the children are being worked on.
